...
from utils.buffer.PSO_utils import compute_PSO
from utils.buffer.PSO_utils import compute_mean_feature
from utils.setup_elements import n_classes
from utils.utils import nonzero_indices, maybe_cuda


class PSO_update(object):
    ...
    def update(self, buffer, x, y, **kwargs):
        ...
        category_seen_so_epoch = torch.tensor([0]*100)
        for i in set(y):
            category_seen_so_epoch[i-1] =category_seen_so_epoch[i-1]+ sum(y==i).to(category_seen_so_epoch.device)
        buffer.n_category_seen_so_far += category_seen_so_epoch
        # print("all",buffer.n_category_seen_so_far)
        # print("epoch",category_seen_so_epoch)
        # 计算并更新样本在特征图中的平均中心
        compute_mean_feature(buffer, x, y)
        ...
        if buffer.current_index == self.mem_size:
            ...
            self._update_by_PSO(model, buffer, cur_x, cur_y)

    def _update_by_PSO(self, model, buffer, cur_x, cur_y):
        """
        ...
        ret_x,ret_y = compute_PSO(buffer,model, cand_x, cand_y, device=self.device)
