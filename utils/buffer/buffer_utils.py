...
def random_retrieve(buffer, num_retrieve, excl_indices=None, return_indices=False):
    ...
    # if x.size()[0]>0:
    #     down_imgs =list()
    #     #cv库的图片的三个维度与pytorch不太一样，需要进行交换
    #     x_ = x.cpu().numpy().swapaxes(1,2).swapaxes(2,3)
    ...
    #         down_imgs.append(i)
    #         # print("di:",i.shape)
    #     x = torch.tensor(np.array(down_imgs).swapaxes(2,3).swapaxes(1,2)).cuda()
    #     # x = torch.cat((x, x))
    #     print("x:",x.size())
