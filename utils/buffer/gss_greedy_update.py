    ...
    def update(self, buffer, x, y, **kwargs):
        ...
        if place_left <= 0:  # buffer is full
            ...
            if batch_sim < 0:
                ...
                buffer.buffer_img[index[sub_index.to(index.device)]] = x[added_indx[sub_index.to(added_indx.device)]].clone()
                buffer.buffer_label[index[sub_index.to(index.device)]] = y[added_indx[sub_index.to(added_indx.device)]].clone()
                self.buffer_score[index[sub_index.to(index.device)]] = batch_item_sim[added_indx[sub_index.to(added_indx.device)]].clone()
        else:
