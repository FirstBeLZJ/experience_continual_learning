...
class LZJ_retrieve(object):
    ...
    def retrieve(self, buffer, **kwargs):
        ...
        if buffer.n_seen_so_far <= self.mem_size:
            ...
            ret_x, ret_y = self._retrieve_by_topsis(model, buffer_x, buffer_y, cur_x, cur_y, self.num_retrieve)
        return ret_x, ret_y

    def _retrieve_by_topsis(self, model, buffer_x, buffer_y, cur_x, cur_y, num_retrieve):
        """
        ...
        cand_x, cand_y = buffer_x,buffer_y
        # 计算eval_x的深度特征的中间，并根据距离给cand_x排序,返回排序后的cand_x 和 cand_y
        ret_x,ret_y = compute_topsis(model, eval_adv_x, eval_adv_y, cand_x, cand_y, device=self.device)
