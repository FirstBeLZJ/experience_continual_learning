...
class PSO_retrieve(object):
    ...
    def retrieve(self, buffer, **kwargs):
        ...
        if buffer.n_seen_so_far <= self.mem_size:
            ...
            ret_x, ret_y = self._retrieve_by_PSO(buffer,model, buffer_x, buffer_y, cur_x, cur_y, self.num_retrieve)
        return ret_x, ret_y

    def _retrieve_by_PSO(self, buffer,model, buffer_x, buffer_y, cur_x, cur_y, num_retrieve):
        """
        ...
        # 计算eval_x的深度特征的中间，并根据距离给buffer_x排序,返回排序后的buffer_x 和 buffer_y
        ret_x,ret_y = compute_PSO(buffer,model, buffer_x,buffer_y, device=self.device)
        #print("=====retrieve_result:",ret_x.size(),ret_y.size())
        ...
        ret_x = ret_x[:num_retrieve]
        ret_y = ret_y[:num_retrieve]
        return ret_x, ret_y
